...
for airfoil in airfoils.values():
    ...
    # Calculate dCp for panel method for AoA = 10 degrees
    x, y = generate_naca4_contour(airfoil.code, n_points=51)
    ...
    airfoil.dCp_xc_pm = results["xp"]
    airfoil.dCp_panel_method = results["Cp"]

    

    # access xfoil results from dictionary to get dCp and xc for both free and fixed transition
    airfoil.dCp_xfoil_free = xfoil_results[airfoil.code]["free"]["dCp"]
    airfoil.dCp_xfoil_fixed = xfoil_results[airfoil.code]["fixed"]["dCp"]
    airfoil.dCp_xc_xfoil_free = xfoil_results[airfoil.code]["free"]["x_c"]
    airfoil.dCp_xc_xfoil_fixed = xfoil_results[airfoil.code]["fixed"]["x_c"]
...
plot_flexible(
    x_val=[
        [af.dCp_xc_tat, af.dCp_xc_pm, af.dCp_xc_xfoil_free, af.dCp_xc_xfoil_fixed]
        for af in airfoils.values()
    ],
    y_vals=[
        [af.dCp_thin_airfoil, af.dCp_panel_method, af.dCp_xfoil_free, af.dCp_xfoil_fixed]
        for af in airfoils.values()
    ],
    labels=[
        ["Thin Airfoil", "Panel Method", "XFOIL Free", "XFOIL Fixed"]
        for af in airfoils.values()
    ],
    x_label="x/c [-]",
...
# plot the panel method for each airfoil on 4 subplots
plot_flexible(
    x_val=[[af.dCp_xc_pm] for af in airfoils.values()],
    y_vals=[[af.dCp_panel_method] for af in airfoils.values()],
